...
`mkwanja-payment-svc` is the central payment processing and double-entry ledger service for all Dexter's and Appify's applications. It is the single point of contact for any money movement — no consumer application ever talks to Safaricom's Daraja API directly.
...
Under the CBK's National Payment System Act and PSP Regulations, a licence is required to operate as a payment intermediary. By requiring each business to hold their own Safaricom till and Daraja credentials, mkwanja acts as a software integration layer rather than a payment intermediary. Each business's M-PESA transactions flow between that business's registered till and Safaricom — mkwanja facilitates the API calls but never holds, settles, or intermediates funds.

This model must be maintained consistently:

- mkwanja must never operate a shared till used across multiple clients
- Each client's Daraja credentials are used exclusively for that client's transactions
- The service must not commingle funds between clients in any way
...
- **Isolation.** Consumer apps never deal with Daraja directly. Each client's credentials and ledger are fully siloed.
...
- No end-user authentication — consumer apps authenticate their users; this service authenticates calling _services_
...
- No UI
- No mkwanja-owned shared till — all Daraja calls use individual client credentials
...
- Their Daraja initiator name and security credential (required for B2C and B2B outbound payments)

The consumer app collects these during its onboarding wizard and calls the payment service credentials registration endpoint. The payment service encrypts credentials at rest and associates them with that `client_id`.
...
- Each client's Daraja OAuth token is cached in Redis under a key scoped to their `client_id` — never shared across clients
...
2. Loads and decrypts the client's credentials from that database
3. Fetches (or uses cached) OAuth token for that specific client
4. Calls Daraja using that client's shortcode and credentials only

### 5.4 Webhook routing

Each consumer app registers its own Safaricom webhook URL path (for example, `/webhooks/mpesa/stk/eazibiz` for eazibiz and `/webhooks/mpesa/stk/mkwanja` for mkwanja). The path identifies the consumer database; within that database the service still needs to identify the exact client and payment. It does this by:

1. At initiation time: storing `stk:{CheckoutRequestID}` → `{clientID}:{paymentID}` in Redis (30-minute TTL)
2. At webhook receipt: using the URL path to select the consumer database, then reading the Redis key to identify the client and payment to update
...
Every consumer database contains the same schema: `clients`, `client_credentials`, `payments`, `payment_events`, `journal_accounts`, `journal`.
...
| Account Balance          | —                   | Check a client's M-PESA till balance                |
...
B2B requires an API initiator configured on the client's Daraja portal. Clients must provide initiator name and security credential during M-PESA setup (optional step — required only if they want outbound B2B payments).
...
### 8.1 Standard journal accounts (seeded per client on registration)
...
| -------- | ------------------------------------ | --------------------------------------------- |
| `POST`   | `/v1/clients`                        | Register client + store encrypted credentials |
| `POST`   | `/v1/clients/test-credentials`       | Verify credentials work without saving        |
| `PUT`    | `/v1/clients/:client_id/credentials` | Update credentials                            |
| `DELETE` | `/v1/clients/:client_id`             | Deactivate client                             |
...
| Method | Path                       | Description                        |
| ------ | -------------------------- | ---------------------------------- |
| `GET`  | `/v1/ledger`               | Query journal entries for a client |
| `GET`  | `/v1/ledger/balance`       | Account balances for a client      |
...
| Method | Path                                        | Description      |
| ------ | ------------------------------------------- | ---------------- |
| `POST` | `/webhooks/mpesa/stk/:consumer_id`          | STK Push result  |
| `POST` | `/webhooks/mpesa/b2c/:consumer_id`          | B2C result       |
| `POST` | `/webhooks/mpesa/b2b/:consumer_id`          | B2B result       |
| `POST` | `/webhooks/mpesa/c2b/:consumer_id/confirm`  | C2B confirmation |
| `POST` | `/webhooks/mpesa/c2b/:consumer_id/validate` | C2B validation   |
...
Background job every 5 minutes: queries any payment in `pending` state older than 2 minutes, calls Daraja Transaction Status API using that client's credentials, updates accordingly. Recovers payments where Safaricom's webhook was never delivered.
...
- One client's credentials can never affect another client's transactions
